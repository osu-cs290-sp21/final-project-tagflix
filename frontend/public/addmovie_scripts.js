...
addMovie.addEventListener("click", () => {
  ...
  if (
    ...
    //checks for all input boxes to have infomation in them
    var request = new XMLHttpRequest();
    request.open(
      "POST",
      "https://us-west-2.aws.webhooks.mongodb-realm.com/api/client/v2.0/app/tagflix-gprxr/service/movies/incoming_webhook/movie-new"
    );
    ...
    for (var i = 0; i < genre_list.length; i++) {
      if (master_genre_list.search(genre_list[i]) == -1) {
        flag = 1;
        alert(
          "Invalid genre detected, please refer to genre dropdown above for valid options"
        );
        break;
      }
    }
